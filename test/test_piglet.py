...
def test_initial_state_and_legal_actions():
    ...
    assert piglet.legal_actions(state) == ("flip", "hold")
    assert piglet.legal_actions(terminal) == ()

    with pytest.raises(ValueError, match="first_player"):
        piglet.initial_state(first_player=-1)
...
def test_hold_banks_turn_total_and_passes_turn():
    ...
    nxt = piglet.step(state, "hold", random.Random(0), target_score=3)
...
def test_hold_that_reaches_target_wins_immediately():
    ...
    nxt = piglet.step(state, "hold", random.Random(0), target_score=2)
...
def test_flip_heads_adds_one_and_keeps_turn():
    ...
    nxt = piglet.step(state, "flip", FixedCoin(0.25), target_score=3)
...
def test_flip_tails_busts_and_passes_turn():
    ...
    state = piglet.PigletState(scores=(0, 0), turn_total=1, current_player=0)
    nxt = piglet.step(state, "flip", FixedCoin(0.75), target_score=3)
...
def test_step_rejects_illegal_action_and_keeps_terminal_state_fixed():
    ...
    state = piglet.initial_state()
    terminal = piglet.PigletState(scores=(2, 0), turn_total=0, current_player=0, winner=0)

    with pytest.raises(ValueError, match="Illegal Piglet action"):
        piglet.step(state, "roll", FixedCoin(0.25))  # type: ignore[arg-type]

    assert piglet.step(terminal, "flip", FixedCoin(0.25), target_score=2) is terminal
...
def test_threshold_and_always_flip_policies_hold_when_win_is_available():
    ...
    threshold_policy = piglet.make_hold_at_policy(threshold=2, target_score=3)
    always_flip_policy = piglet.make_always_flip_policy(target_score=3)
    ...
    with pytest.raises(ValueError, match="threshold"):
        piglet.make_hold_at_policy(threshold=-1)
